...
class ETicketService
{
    ...
    private function parseBookingData(string $kodeBooking): ?array
    {
        try {
            ...
            $paket = $pemesanan->paketWisata ?? $pemesanan->jadwal?->paket;
            $tanggalKunjungan = $pemesanan->tanggal_kunjungan ?? $pemesanan->jadwal?->tanggal;

            if (! $paket || ! $tanggalKunjungan) {
                throw new \Exception('Data paket atau tanggal kunjungan tidak lengkap');
            }
            ...
            return [
                ...
                'package_details' => $paket->deskripsi ?? 'Paket wisata ke The Waterfall',
                'visit_date' => $tanggalKunjungan->format('d F Y'),
